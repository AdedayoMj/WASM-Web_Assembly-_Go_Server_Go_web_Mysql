...
// EmailConfig common shiga email configurations
...
func SendEmailSendGrid(e EmailConfig) (bool, error) {
	...
	case EmailFormatNewUser:
		newEmailContent = ShigaHTMLHeader + NewUserActivation(e.EmailConfirmationURL, e.UserName,
			e.SiteName, e.SiteSupportEmail) + ShigaHTMLFooter
	}

	htmlContent, err := sulat.SetHTML(&sulat.EmailHTMLFormat{
		IsFullHTML: false,
		HTMLHeader: ShigaHTMLHeader,
		HTMLBody:   newEmailContent,
		HTMLFooter: ShigaHTMLFooter,
	})
